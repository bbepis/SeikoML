	...
	public class Hooks : BaseUnityPlugin
	{
		public void Awake()
		{
			...
			IL.RoR2.SurvivorCatalog.cctor += (IL) =>
			{
				...
				c.EmitDelegate<Func<SurvivorIndex[], SurvivorIndex[]>>(r => Api.BuildIdealOrder(r));
			};
			...
			On.RoR2.SurvivorCatalog.GetSurvivorDef += (Orig, survivorIndex) =>
			{
				...
				if (survivorIndex < 0 || (int)survivorIndex > SurvivorCatalog.survivorDefs.Length)
				{
					return null;
				}
			...
			IL.RoR2.SurvivorCatalog.Init += (il) =>
			{
				...
				c.EmitDelegate<Func<int>>(Api.GetSurvivorCount); //Set Survivor Array to custom amount
			...
			On.RoR2.SystemInitializerAttribute.Execute += (orig) => { orig(); };
		}

		private int VerifyMaxSurvivors(int current)
		{
			if (current < Api.GetSurvivorCount())
				return Api.GetSurvivorCount();
			else
				return current;
		}
	}
